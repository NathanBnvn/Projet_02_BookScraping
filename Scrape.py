...
navigation_panel = soup.find('ul', {'class': 'nav nav-list'}).select('a')
...
with open('categories links.txt', 'r') as categories_links_file:
	for classification in categories_links_file:
		...
		if response1.ok:
			...
			# Verifier s'il y a une nouvelle page en fonction du nombre de livre
		...
		# Scrape l'ensemble des données d'un livre
			...
			with open('links.txt', 'r') as urls_file:
				with open('test_category.csv', 'w') as output_file:
